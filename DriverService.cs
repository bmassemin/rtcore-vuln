...
internal class DriverService(ILogger logger)
{
    ...
    /// </summary>
    public void InstallDriver(string path, string serviceName)
    {
    ...
    public CloseServiceHandleSafeHandle OpenServiceByName(string serviceName)
    {
        using var scmHandle = OpenSCManager(null, null, SC_MANAGER_ALL_ACCESS);
        if (scmHandle.IsInvalid)
            throw new Win32Exception("Failed to open service control manager.");
        var serviceHandle = OpenService(scmHandle, serviceName, SERVICE_ALL_ACCESS);
        if (serviceHandle.IsInvalid)
            throw new Win32Exception($"Failed to open service '{serviceName}'.");
